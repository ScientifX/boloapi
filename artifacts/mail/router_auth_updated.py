"""
Authentication Router (Updated with Email Integration)
...
async def register(request: Request, register_req: RegisterRequest):
    ...
    try:
        ...
        if existing_user:
            if existing_user['is_active']:
                ...
                response_note = (
                    ...
                    else f"Email not configured. For testing: /auth/activate?token={activation_token}"
                )
                
                return RegisterResponse(
                    message="Activation email resent" if email_sent else "Registration successful (email disabled)",
                    user_id=str(user_id),
                    email=email,
                    note=response_note
                )
        ...
        response_note = (
            "Check your email for the activation link." if email_sent
            else f"For testing, activate at: /auth/activate?token={activation_token}"
        )
...
@limiter.limit(rate_max)
async def activate(request: Request, token: str = Field(..., description="Activation token from email")):
    """
    ...
    try:
        ...
        return ActivateResponse(
            ...
            api_key=api_key,
            instructions="Save this API key securely. Use it with /auth/token to get access tokens. You won't be able to see this key again. A copy has been sent to your email."
        )
...
async def reset_api_key(request: Request, register_req: RegisterRequest):
    ...
    try:
        ...
        instructions = (
            ...
            if email_sent else
            "Your old API key and all tokens generated from it are now invalid. "
            "Save this new key securely. (Email disabled - key shown here only)"
        )
        
        return ResetKeyResponse(
            message="API key reset successful" + (" - check your email" if email_sent else ""),
            api_key=api_key,
